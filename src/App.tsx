...
function App() {
  const [activeTab, setActiveTab] = useState<'convert' | 'translate' | 'auto' | 'tools'>('convert')
    ...
    <div className="min-h-screen p-4 md:p-8 relative z-0">
      ...
      <header className="max-w-6xl mx-auto mb-12 relative">
        <div className="neo-card bg-primary inline-block mb-4 rotate-[-2deg]">
          <h1 className="text-4xl md:text-6xl tracking-tighter">Subtitle Tool</h1>
        </div>
        ...
        <div className="inline-block bg-black text-white neo-border px-4 py-2 mt-2 -rotate-1">
          <p className="text-xl font-bold uppercase">Powerful. Private. Neo-Brutalist.</p>
        </div>
      </header>
          ...
          {activeTab === 'auto' && (
            <div className="max-w-md">
               <div className="bg-primary/20 p-8 neo-border border-dashed mb-6 cursor-pointer hover:bg-primary/30 transition-colors">
                ...
                <p className="text-sm text-red-600 font-bold italic">No Upload Required - Local Extraction</p>
              </div>
      ...
      <footer className="max-w-6xl mx-auto mt-12 pt-8 border-t-4 border-black flex justify-between items-center text-white">
        <p className="font-bold uppercase drop-shadow-[2px_2px_0px_rgba(0,0,0,1)]">© 2026 SubtitleApp</p>
        <div className="flex gap-4">
          <div className="neo-card p-2 bg-white text-black text-xs font-bold">LOCAL-ONLY</div>
          <div className="neo-card p-2 bg-white text-black text-xs font-bold">NO DATABASE</div>
        </div>
      </footer>
    </div>
  )
